...
However, we set the resulting node's next value to point to the returned value of calling our recursive algorithm with the next value node of one linked list pointer (the one the exits), the calculated carry value, and a new empty node.
...
We can compute a binary search but also pass in the current row into the binary method.

This way, when the target is greater than the last element in the current row, we can then search the next row (if there are more rows) and p